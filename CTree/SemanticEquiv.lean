  ...
  theorem refine_of_weak_sim {sim : Rel (CTree ε ρ) (CTree ε ρ)} {t1 t2 : CTree ε ρ}
    ...
    case vis =>
      ...
      case ret =>
        sorry
      case vis =>
        if hαe : α2 = α ∧ HEq e2 e then
          obtain ⟨hα, he⟩ := hαe
          subst hα he
          apply RefineF.vis; intro a
          obtain ⟨t2', hws, hcont⟩ := hsim (vis e2 k) (vis e2 k2) h (.event α2 e2 a) (k a) .vis
          apply RefineF.coind 0 0 ENat.top_pos ENat.top_pos
          exact hws.vis_event hsim hcont
        else
          exfalso
          exact weak_sim_event_ne hsim h hαe
      case tau =>
        sorry
      case zero =>
        sorry
